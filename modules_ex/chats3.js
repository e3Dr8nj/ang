...
module.exports.e={
  ...
  main_voice_id:'730867064784420883'
...
exports.onVoiceClose=async(client,oldMember,newMember)=>{try{ //triggered them last mmb left the voice channel
           ...
           if(!exports.voice_channels[channel.id]) return;
           if(exports.voice_channels[channel.id].ud==1) return;
           if(channel.members.array().length!=0) return console.log('there are members inside');
...
exports.createNewVoice=async(client,oldMember,newMember)=>{try{ //triggered then new chat creating
 ...
 let sv_name=await channel.name;await channel.edit({name:exports.e.no_rights_for_creating}).catch(console.error);await delay(exports.e.delay_time);await channel.edit({name:sv_name}).catch(console.error);
         ...
         return console.log('has chat already');};
...
//____________LINK FREE TEXT CHAT
       ...
       let free_chat=await channel.guild.channels.find(ch=>ch.name.startsWith('текстовый')); console.log('find free');
            if(!free_chat){console.log('no free text'); return;};
//__________________CREATE VOICE CHANNEL
        let new_channel=await channel.clone(undefined).catch(console.error);
        await new_channel.setBitrate(8000).catch(console.error);
      ...
      let num = free_chat.name.match(/\d{1,}/)[0]; if(!num) return; console.log(num);
      let role = await free_chat.guild.roles.find(r=>r.name=='роль'+num); 
      if(!role) return; console.log('no role');
     //_____________________
         
      // let data = "ch:"+new_channel.id+" ow:"+member.user.id+" rl:"+role.id+" ud:0 opened:0 blocked:0";
         let data={ch:new_channel.id,ow:member.user.id,rl:role.id,ud:0,opened:0,blocked:0};
        //await free_chat.edit({topic:data,name:name}).catch(console.error);
        let logbot_channel = free_chat.guild.channels.find(ch=>ch.name=='logbot');
        logbot_channel.send(JSON.stringify(data));
