...
//console.log(oldMember.voiceState);
  if(exports.logs) console.log('inc');
   if (!oldState || !newState) return;

  const oldChannel = oldState.channel ? oldState.channel : null;
  const newChannel = newState.channel ? newState.channel : null;
  if(oldState.id==newState.id) console.log('same');
  let oldMember=oldState.member;
  let newMember = newState.member;
  let from = !!oldChannel;
let to = !!newChannel;
if(from) console.log('from '+oldChannel.parent.id);
if(to) console.log('to '+newChannel.parent.id);
  
//if (!(from&&oldChannel.parent.id==exports.e.voice_category_id||to&&newChannel.id==exports.e.voice_category_id)) return console.log('no one');
let inside = newChannel!=null&&newChannel.parent.id==exports.e.voice_category_id;
let out = oldChannel!=null&&oldChannel.parent.id==exports.e.voice_category_id;
  if(!(inside||out)) return;
if (inside) {console.log('inside');};
if(out){console.log('out')};
  let income = (newChannel&&exports.voice_channels[newChannel.id])?newChannel.id:false; 
  let outcome = (oldChannel&&exports.voice_channels[oldChannel.id])?oldChannel.id:false;
  let create_voice_income = !!(newChannel&&newChannel.id==module.exports.e.main_voice_id);
  if(!(income||outcome||create_voice_income||out)) {return;};
  if (exports.logs) console.log('connect');
  let member = newMember.guild.members.cache.get(newMember.user.id);
  
  //DISCONNECT CASE
   //--
   if(out&&!outcome&&(oldChannel&&oldChannel.id!=module.exports.e.main_voice_id)) { 
      if(oldChannel.members.cache.array().length==0) {console.log('del'); await oldChannel.delete();};
 };
  if(outcome) { 
        // await exports.onDisconnect(client,oldMember,newMember);
         ...
         if(voiceChannel.members.cache.array().length==0) {module.exports.onVoiceClose(client,oldMember,newMember);};
         
    };
 
//CONNECT CASE
  if(!!newChannel) {
     if(create_voice_income){console.log('crt v'); return exports.createNewVoice(client,oldState,newState);};//if new chat creation inicialized
     if(income){ 
                //return exports.onConnect(client,oldMember,newMember);
                let voiceChannel=await member.guild.channels.cache.get(income);
           await member.addRole(member.guild.roles.cache.get(exports.voice_channels[income].role_id)).catch(console.error);
...
module.exports.events.channelUpdate={ on:true,  run:async(client,oldChannel,newChannel)=>{try{//synch voice channel name and role name
     ...
     let text_chnl= await oldChannel.guild.channels.cache.get(ch_cast.text_channel.id);
     text_chnl.edit({name:newChannel.name}).then().catch(console.error);
     let role=await oldChannel.guild.roles.cache.get(ch_cast.role_id); if(!role) return;
...
exports.onConnect=async(client,oldMember,newMember)=>{try{ 
         ...
         let role = channel.guild.roles.cache.get(role_id);
         if(role) try{newMember.rolse.add(role).then().catch(err=>console.log(err)); }catch(err){console.log(err);};
        return;
...
exports.onDisconnect=async(client,oldMember,newMember)=>{try{ //triggered then mmb left the voice channel
         ...
         let role_id=exports.voice_channels[channel.id].role_id;
         let role = channel.guild.roles.cache.get(role_id);
         if(role) try{oldMember.roles.remove(role).then().catch(err=>console.log(err)); }catch(err){console.log(err);};
         return;
...
exports.onVoiceClose=async(client,oldMember,newMember)=>{try{ //triggered them last mmb left the voice channel
           ...
           let text_channel=await member.guild.channels.cache.find(ch=>ch.topic&&ch.topic.indexOf('ch:'+channel.id)!=-1);
            if(text_channel){ 
                 await text_channel.edit({name:'fch'+role_id,topic:''});
                 let parent = await member.guild.channels.cache.get(exports.e.free_chat_category_id);
                 if(parent) {await text_channel.setParent(parent.id).then(channel => { channel.lockPermissions().catch(console.error); }).catch(console.error);};
...
exports.createNewVoice=async(client,oldState,newState)=>{try{ //triggered then new chat creating ---
         if(exports.logs) console.log("create new voice");
...
//____________LINK FREE TEXT CHAT
       let free_chat=await channel.guild.channels.cache.find(ch=>ch.name.startsWith('fch')); console.log('find free');
            if(!free_chat){
...
//__________________CREATE VOICE CHANNEL
      ...
      let role = await free_chat.guild.roles.cache.get(role_id); 
...
exports.onChatBlockPerms=async(client,text_chat,voice_chat)=>{try{ //on block
        //  await free_chat.overwritePermissions(mmb.user,{VIEW_CHANNEL:true,SEND_MESSAGES:true}).catch(console.error);
          ...
          let role_visible=await voice_chat.guild.cache.roles.find(r=>r.name==exports.e.sp_role_name); if(!role_visible) return; console.log('v b');
...
exports.onChatBlockPerms2=async(client,text_chat,voice_chat)=>{try{ //on block
        //  await free_chat.overwritePermissions(mmb.user,{VIEW_CHANNEL:true,SEND_MESSAGES:true}).catch(console.error);
          await voice_chat.overwritePermissions(voice_chat.guild.defaultRole, { CONNECT: false }).catch(console.error);
          let role_visible=await voice_chat.guild.roles.cache.find(r=>r.name==exports.e.sp_role_name); if(!role_visible) return; console.log('v b');
          await text_chat.overwritePermissions(role_visible, { VIEW_CHANNEL:null }).catch(console.error); console.log('blocked');
...
exports.onChatUnBlockPerms=async(client,text_chat,voice_chat)=>{try{ //on text chat block
        //  await free_chat.overwritePermissions(mmb.user,{VIEW_CHANNEL:true,SEND_MESSAGES:true}).catch(console.error);
          ...
          let role_visible=await voice_chat.guild.roles.cache.find(r=>r.name==exports.e.sp_role_name);
...
//_____________voice    
     ...
     if(args[1]=='-войс'){//mute on voice channel
       ...
       if(item_mmb.username&&voice_channel.members.cache.get(item_mmb.id)) {await message.guild.members.get(item_mmb.id).setVoiceChannel(afk).catch(console.error);
  await message.guild.members.get(item_mmb.id).setVoiceChannel(voice_channel).catch(console.error); };
        ...
        if(item_mmb.username&&voice_channel.members.cache.get(item_mmb.id)) {await message.guild.members.cache.get(item_mmb.id).setVoiceChannel(afk).catch(console.error);};
        return;
...
//_______
   ...
   if (users_arr) users_arr.map(u=>{
     let sp=message.guild.members.cache.get(u.id)&&message.guild.members.cache.get(u.id).roles.get('684149811032621105');
